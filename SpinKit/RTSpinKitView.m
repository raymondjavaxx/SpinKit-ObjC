...
-(id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    return [self initWithStyle:RTSpinKitViewStylePlane color:[UIColor grayColor]];
}
...
-(instancetype)initWithStyle:(RTSpinKitViewStyle)style color:(UIColor*)color {
    ...
    if (CGRectIsEmpty(self.frame)) {
        self = [super initWithFrame:CGRectMake(0, 0, kIdealSize, kIdealSize)];
    }
...
-(CGSize)sizeThatFits:(CGSize)size {
    if (CGRectIsEmpty(self.frame)) {
        return CGSizeMake(kIdealSize, kIdealSize);
    }
